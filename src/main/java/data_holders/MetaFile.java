...
//		}
		...
		for(Beam b1: beams.values()) {
			...
			for(Beam b2: beams.values()) {
				...
				beamResponses.putIfAbsent(b2, Math.exp(d));
				
			}
			
		    b1.addNeighbours(beamResponses.entrySet().stream().sorted(Map.Entry.comparingByValue()).collect(Collectors.toList()).subList(0, 6).stream().map(f -> f.getKey()).collect(Collectors.toList()));
